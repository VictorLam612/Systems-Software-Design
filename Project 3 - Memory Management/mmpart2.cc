...
int main(int argc, char** argv) {
    ...
    for(int j = 0; j < 32; j++){
     ...
     for(int k = 0; k < 32; k++){
        physical_frames[k].valid = 0;
    }
    // Set first frame for OS Only
    physical_frames[0].valid = 1;
    physical_frames[0].time_of_use = 0xffffffff;
    ...
    FILE * fd2 = fopen("output-part2", "wb");

    for (int i = 0; i < fileSize/sizeof(unsigned long long); i++) {
        // %lx prints unsigned long int in hex
        
        unsigned long temp = memAccesses[i] >> 7;
        printf("%i: %lu ", i, temp);
        // temp = virtual page number, need to find which physical frame holds it
        unsigned long foo = request(temp); //page_table[temp] * page_size;

        unsigned long long bar = memAccesses[i] & 127;
        unsigned long long foobar = foo*page_size + bar;
        printf("%lu ", foo);
        printf("\n");
